...
import { getAuthen, getPrivilegesService } from "../service"
...
export default function SigninForm() {
  ...
  const succeed = (result: AuthResult) => {
    ...
    getPrivilegesService()
      ...
      .finally(() => router.push(storage.home))
  }
  const signin = async (event: OnClick) => {
    ...
    try {
      ...
      const res = await authenticator.authenticate(user)
      const s = res.status
      if (s === status.two_factor_required) {
        ...
        if (res.user) {
          const expiredDays = dayDiff(res.user.passwordExpiredTime, new Date())
    ...
    <div className="view-container central-full sign-in-view-container">
      <form id="signinForm" name="signinForm" noValidate={true} autoComplete="off" ref={form as any}>
        <div>
          ...
          <Link id="btnForgotPassword" href="/authentication/forgot-password">
            {resource.button_forgot_password}
          </Link>
          <Link id="btnSignup" href="/authentication/signup">
