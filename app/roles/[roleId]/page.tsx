      ...
      return (
        <section className="col s12" key={m.id}>
          <label className="checkbox-container">
            <input type="checkbox" name="modules" disabled={disableds} data-id={m.id} data-type="parent" checked={checked} onChange={handleCheck} />
            {m.name}
          </label>
          <section className="row checkbox-group">{renderForms(roles, m.children, m.id, disableds, allPrivilege)}</section>
          <hr />
        </section>
      )
    } else {
    ...
    <div className="view-container">
      <form id="roleForm" name="roleForm" model-name="role" ref={refForm as any}>
        <header>
          ...
          <h2>
            {flag.newMode ? resource.create : resource.edit} {resource.role}
          </h2>
          <button className="btn-group btn-right">
            <i onClick={(e) => assign(e, role.roleId)} className="material-icons">
              group
            </i>
          </button>
        </header>
        <div>
          ...
          <h4>
            <label>
              <input type="checkbox" value="all" disabled={state.keyword !== ""} data-type="all" checked={state.checkedAll} onChange={handleCheckAll} />
          ...
          <section className="row hr-height-1">{renderForms(state.role, state.shownPrivileges, "", state.keyword !== "", state.allPrivileges)}</section>
        </div>
        <footer>
          {!flag.readOnly && (
